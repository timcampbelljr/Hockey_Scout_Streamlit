...
def render_player_card(player_name, player_stats, player_shots, faceoff_data, shootout_data, games_df):
    ...
    with tab1:
        ...
        if not player_games.empty and not games_df.empty:
            ...
            st.dataframe(
                player_games[["game_id", "opponent", "goals", "assists", "points", "plus_minus", "penalty_minutes", "shots"]].sort_values("game_id", ascending=False),
                hide_index=True,
...
def render_goalie_card(goalie_name, goalie_stats, goalie_shots, shootout_data, games_df):
    ...
    with tab1:
        ...
        if not goalie_games.empty and not games_df.empty:
            ...
            st.dataframe(
                goalie_games[["game_id", "opponent", "saves", "goals_against", "save_percentage_game"]].sort_values("game_id", ascending=False),
                hide_index=True,
                use_container_width=True,
                column_config={
                    "game_id": "Game",
